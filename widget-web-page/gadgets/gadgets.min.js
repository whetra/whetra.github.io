...
}function isTestServer(){return document.location.host.indexOf("localhost")>-1||document.location.host.indexOf("preview")>-1||document.location.host.indexOf("viewer-test")>-1}gadgets.config.register("core.io",null,init);return{makeRequest:function(url,callback,opt_params){var urlParams=gadgets.util.getUrlParameters();
...
}}}}st=st||shindig.auth.getSecurityToken();var opt_headers=st?{"X-Shindig-ST":st}:{};var defaultUrl = document.location.host === 'viewer.risevision.com' ? "http://viewer.risevision.com" : "http://rvashow.appspot.com";var proxyHost=isTestServer()?"//"+document.location.host:defaultUrl;var proxyUrl=proxyHost+"/gadgets/makeRequest";if(!respondWithPreload(paramData,params,callback)){if(httpMethod=="GET"&&typeof(refreshInterval)!="undefined"){paramData.refresh=refreshInterval
